...
define([
    ...
    configSymbo
      ...
      _init : function() {
         ...
         if(configSymbo.symbols.length>0){
      ...
      _multiScrollToPage: function(index){
...
//               domClass.add(pageSlider, 'pageSlider');
      ...
      _animateScroll : function(start, end) {
         ...
         var me = this;
          var nb = 1;
          var box = html.getContentBox(dom.byId("panelContent"));
          if(Math.abs(start-end)<(this.pages.length * box.h)){
              nb += (Math.abs(start-end)/box.h)/this.pages.length;
          }
      ...
      _renderResults : function(pageObj) {
            ...
            if (pageObj.proximityFeatures) {
               ...
               for (var i=0; i<pageObj.proximityFeatures.length; i++) {
                   ...
                   var symbol;
                   if(this.symbology){
                       symbol = this.symbology.getSymbol(gra);
                       if(symbol)
                           symM = symbol;
                   }
